<template>
  <div>
    <section id="services-section" class="px-12 lg:px-32 py-16 relative overflow-hidden">
      <!-- Text wrapper start -->
      <div class="flex gap-8 flex-col md:flex-row">
        <h1 class="uppercase text-5xl mb-4 font-semibold ">
          我們的服務
        </h1>
        <p class="xl:w-1/2 text-stone-500 mb-8">
          我的專業能幫你做出有質地的影像成品。
          專門協助宅產業類型客戶提供影像製作服務。
          7 個工作天提供您成品，並調整符合到您滿意的狀態。<br />
          <br />
          My specialty can help you make textured finished products.
          Specializing in assisting home industry customers to provide video production services.
          7 working days to deliver your finished product and adjust it to your satisfaction.
        </p>
      </div>
      <!--  -->
      <div class="flex flex-col md:flex-row md:gap-8">
        <div class="flex flex-row md:flex-col justify-between md:justify-start items-center md:items-start my-8 md:w-1/3">
          <img src="@/assets/services/lifestyle.png" alt="interior" class="-z-50 md:mb-4">
          <div class="w-2/3 text-right md:text-left md:w-full">
            <a href="https://www.behance.net/gallery/158438305/40-" target="_blank">
              <h1
                class="uppercase text-2xl font-semibold hover:text-yellow-400 transition delay-80 duration-150 ease-in-out">
                室內空間作品攝影
              </h1>
            </a>
            <p class="capitalize text-stone-500">
              室內設計事務所完工作品集紀錄
            </p>
            <p class="capitalize text-stone-500">
              Interior Design Office Completed Portfolio Record
            </p>
          </div>
        </div>
        <div class="flex flex-row md:flex-col justify-between items-center md:items-start my-8 md:w-1/3">
          <div class="w-2/3 text-left md:w-full md:order-2">
            <a href="https://www.behance.net/gallery/131481845/_" target="_blank">
              <h1
                class="uppercase text-2xl font-semibold hover:text-yellow-400 transition delay-80 duration-150 ease-in-out">
                建材公司影片
              </h1>
            </a>
            <p class="capitalize text-stone-500">
              室內設計訪談影片
            </p>
            <p class="capitalize text-stone-500">
              Interior Design Interview Videos And Room Tour
            </p>
          </div>
          <img src="@/assets/services/moments.png" alt="film" class="-z-50 md:mb-4 md:order-1">
        </div>
        <div class="flex flex-row md:flex-col justify-between items-center md:items-start my-8 md:w-1/3">
          <div class="w-1/3">
            <img src="@/assets/services/home.png" alt="Moments" class="-z-50 md:mb-4">
          </div>
          <div class="w-2/3 text-right md:text-left md:w-full">
            <a href="https://docs.google.com/forms/d/e/1FAIpQLSf9Gz3Tbm2qxpIxiQVAZRqNx3ZBaO2R3LiQam-Wpyt-xno_kA/viewform"
              target="_blank">
              <h1
                class="uppercase text-2xl font-semibold hover:text-yellow-400 transition delay-80 duration-150 ease-in-out">
                屋主新居落成家族紀錄片
              </h1>
            </a>
            <p class="capitalize text-stone-500">
              屋主新屋紀錄片(限台北市)
            </p>
            <p class="capitalize text-stone-500">
              The homeowner's new house record commission (Taipei City)
            </p>
          </div>
        </div>
      </div>
      <!-- Circle start -->
      <div class="bg-yellow-400 h-64 w-64 rounded-full absolute top-0 -right-20 mt-16 -z-20"></div>
    </section>
    <section id="positive-energy" class="px-12 lg:px-32 py-16 relative overflow-hidden">
      <div class="mt-10 flex gap-8 flex-col md:flex-row">
        <h1 class="uppercase text-5xl mb-4 font-semibold ">
          更多居家正能量
        </h1>
        <p class="xl:w-1/2 text-stone-500 mb-8">
          當次的室內攝影委託費，收到費用即捐款 1 成至"家扶扶幼捐款項目"。<br />
          委託者個人平時的捐款也可兌換成攝影服務張數。
          我們嘗試用各種形式，詮釋家的正能量概念，
          讓新居落成的美好，也間接讓未知的家庭受益。
          <br />
        </p>
      </div>
      <!--  -->
      <div class="flex flex-col md:flex-row md:gap-8">
        <div class="flex flex-row md:flex-col justify-between md:justify-start items-center md:items-start my-8 md:w-1/3">
          <img src="@/assets/services/home.png" alt="interior" class="-z-50 md:mb-4">
          <div class="w-2/3 text-right md:text-left md:w-full">
            <a href="https://donate.ccf.org.tw/19/shopcart-step1/210" target="_blank">
              <h1
                class="uppercase text-2xl font-semibold hover:text-yellow-400 transition delay-80 duration-150 ease-in-out">
                家扶扶幼捐款項目
              </h1>
            </a>
            <p class="capitalize text-stone-500">
              每次委託費 1 成捐款至<a href="https://donate.ccf.org.tw/19/shopcart-step1/210">"家扶扶幼捐款項目"</a>，收到委託人的匯款當下即捐出，並截圖給委託人看，創造社會善循環
            </p>          
          </div>
        </div>
        <div class="flex flex-row md:flex-col justify-between items-center md:items-start my-8 md:w-1/3">
          <div class="w-2/3 text-left md:w-full md:order-2">
            <a href="https://www.ccf.org.tw/support/project" target="_blank">
              <h1
                class="uppercase text-2xl font-semibold hover:text-yellow-400 transition delay-80 duration-150 ease-in-out">
                家扶專案捐款兌換攝影
              </h1>
            </a>
            <p class="capitalize text-stone-500">
              委託設計師在奇數月份捐款家扶，每1500元捐款可兌換一張室內攝影<br />
              (須提供佐證為捐款者本人)
            </p>
          </div>
          <img src="@/assets/services/home.png" alt="film" class="-z-50 md:mb-4 md:order-1">
        </div>
        <div class="flex flex-row md:flex-col justify-between items-center md:items-start my-8 md:w-1/3">
          <div class="w-1/3">
            <img src="@/assets/services/home.png" alt="Moments" class="-z-50 md:mb-4">
          </div>
          <div class="w-2/3 text-right md:text-left md:w-full">
            <a href="https://donations.hondao.org.tw/" target="_blank">
              <h1
                class="uppercase text-2xl font-semibold hover:text-yellow-400 transition delay-80 duration-150 ease-in-out">
                弘道老人捐款兌換攝影
              </h1>
            </a>
            <p class="capitalize text-stone-500">
              委託設計師在偶數月份捐款弘道老人福利基金會不限專案，每1500元捐款可兌換一張室內攝影<br />
              (須提供佐證為捐款者本人)
            </p>
          </div>
        </div>
      </div>
    </section>
  </div>
</template>

<script>
export default {

}
</script>

<style></style>